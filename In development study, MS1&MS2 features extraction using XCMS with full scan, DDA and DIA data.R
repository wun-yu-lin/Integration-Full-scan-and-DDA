...
pd <- data.frame(sample_name = sub(basename(MSdata), 
                                   ...
                                   replacement = "", 
                                   fixed = TRUE),
                 sample_group = rep("Fullscan", 3),
                 class=rep("Fullscan", 3),
                 stringsAsFactors
                 = FALSE)
...
#######################################pre-processing "DIA" data, exoprt MS1 features#######################################
MSdata <- paste0("DIA",list.files(path = "DIA",pattern = ".mzXML$", recursive = TRUE))
pd <- data.frame(sample_name = sub(basename(MSdata), 
                                   pattern = ".mzXML",
                                   replacement = "", 
                                   fixed = TRUE),
                 sample_group = rep("DIA", 3),
                 class=rep("DIA", 3),
                 stringsAsFactors
                 = FALSE)
rawData <- readMSData(MSdata, centroided. = TRUE, mode = "onDisk",
